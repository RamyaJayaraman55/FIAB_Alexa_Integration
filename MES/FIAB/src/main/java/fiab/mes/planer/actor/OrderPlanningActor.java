...
public class OrderPlanningActor extends AbstractActor {
    ...
    private Object List;
    private Object AvailableOrderProcessWrapper;
    ...
    public Receive createReceive() {
        return receiveBuilder()
                ...
                //changes begins
                .match(OrderAvailabilityRequest.class, orderWithProcessStepDetails -> {
                    handleProcessStepsMachineCapabilityRequest(orderWithProcessStepDetails);
                })
                .match(OrderAllocationRequest.class, orderAllocationRequest -> {
                    handleOrderAllocationRequest(orderAllocationRequest);
                })
                //changes ends
                .build();
    }
    ...
    //changes begins
    private List<OrderAvailabilityRequest> handleProcessStepsMachineCapabilityRequest(OrderAvailabilityRequest processStepWithMachineCapabilityRequest) {
        String orderId = processStepWithMachineCapabilityRequest.getOrderId();

        List<OrderAvailabilityRequest> orderAvailabilityResponse = new ArrayList<>();
        if (!ordMapper.getOrderRequest(orderId).isPresent()) {
            processStepWithMachineCapabilityRequest.setOrderId(orderId);
            processStepWithMachineCapabilityRequest.setAvailableProcessStepsResponse(String.format("%s order id is not present , incorrect order id", orderId));
            processStepWithMachineCapabilityRequest.setMachineCapabilityResponse("There are no machines available for incorrect order id");

        } else {
            ordMapper.getOrderRequest(orderId).ifPresent(rpr ->
            {
                OrderProcess Process = rpr.getProcess();

                List<CapabilityInvocation> stepCandidates = Process.getAvailableSteps().stream()
                ...
                if (!stepCandidates.isEmpty()) {
                    ...
                    Iterator<CapabilityInvocation> iterator = stepCandidates.iterator();
                    if (iterator.hasNext()) {

                        CapabilityInvocation ci = iterator.next();
                        if (ci.getID() != null) {
                            processStepWithMachineCapabilityRequest.setStepId(ci.getID());
                            processStepWithMachineCapabilityRequest.setStepName(ci.getDisplayName());
                            processStepWithMachineCapabilityRequest.setAvailableProcessStepsResponse(String.format("The available process steps are %s %s for orderId %s ", ci.getID(), ci.getDisplayName(), orderId));
                            processStepWithMachineCapabilityRequest.setOrderId(orderId);
                            processStepWithMachineCapabilityRequest.setStepStatus("Available");
                        }
                        ...
                        if (capMap.isEmpty()) {
                            log.warning(String.format("OrderProcess %s has no available CapabilityInvocations steps match a discovered Machine/Actor, Order is paused", orderId));
                            ordMapper.pauseOrder(orderId);
                            processStepWithMachineCapabilityRequest.setMachineCapabilityResponse(String.format("OrderProcess %s has no available CapabilityInvocations steps that match a discovered Machine/Actor, Order is paused. process step %s cannot be executed", orderId, ci.getID()));
                            ...
                            maOpt.ifPresent(aa -> {
                                ...
                                processStepWithMachineCapabilityRequest.setMachineCapabilityId(aa.getValue());
                                processStepWithMachineCapabilityRequest.setMachineCapabilityResponse(String.format("%s is available for executing process step %s", aa.getValue().getId(), aa.getKey().getID()));
                            ...
                            if (!maOpt.isPresent()) {
                                log.warning(String.format("OrderProcess %s has no available CapabilityInvocations steps match a discovered Machine/Actor, Order is paused", orderId));
                                ordMapper.pauseOrder(orderId);
                                processStepWithMachineCapabilityRequest.setMachineCapabilityResponse(String.format("OrderProcess %s has no available CapabilityInvocations steps match a discovered Machine/Actor, Order is paused, process step %s cannot be executed", orderId, ci.getID()));
                            }
                        }
                    }
                } else if (stepCandidates.isEmpty()) {
                    tryAssignExecutingMachineForOneProcessStep(Process, orderId);
                    processStepWithMachineCapabilityRequest.setOrderId(orderId);
                    processStepWithMachineCapabilityRequest.setAvailableProcessStepsResponse(String.format("There are no process steps available for orderId %s", orderId));
                    processStepWithMachineCapabilityRequest.setMachineCapabilityResponse(String.format("OrderProcess %s has no available CapabilityInvocations steps to map with machine capability", orderId));
                    processStepWithMachineCapabilityRequest.setStepStatus("Not Available");
                }

            });

        }
        //add the order availability responses to list and send to ActorRestEndPoint class
        orderAvailabilityResponse.add(processStepWithMachineCapabilityRequest);
        sender().tell(orderAvailabilityResponse, self);
        return orderAvailabilityResponse;
    }

    private List<OrderAllocationRequest> handleOrderAllocationRequest(OrderAllocationRequest allocationRequest) {
        List<OrderAllocationRequest> orderAllocationResponse = new ArrayList<>();

        String orderId = allocationRequest.getOrderId();
        String machineId = allocationRequest.getMachineId();
        String stepId = allocationRequest.getStepId();
        // check if order id is present
        if (!ordMapper.getOrderRequest(orderId).isPresent()) {

            allocationRequest.setOrderAllocationResponse(String.format("%s order id is not present , incorrect order id", orderId));

        } else {
            //get process for each order id
            ordMapper.getOrderRequest(orderId).ifPresent(rpr -> {
                OrderProcess Process = rpr.getProcess();
                ...
                if (stepCandidates.isEmpty()) {
                    allocationRequest.setOrderAllocationResponse(String.format("Order %s has no available steps for allocation", orderId));
                    tryAssignExecutingMachineForOneProcessStep(Process, orderId);
                } else {
                    // go through each candidate and check if any of the steps can be mapped to a available machine,
                    // first get candidates where the machine capabilites matches
                    ...
                    if (capMap.isEmpty()) {
                        log.warning(String.format("OrderProcess %s has no available CapabilityInvocations steps match a discovered Machine/Actor, Order is paused", orderId));
                        ordMapper.pauseOrder(orderId);
                        allocationRequest.setOrderAllocationResponse(String.format("OrderProcess %s has no available CapabilityInvocations steps that match an available Machine/Actor, Order is paused ", orderId));
                        ...
                        //assign particular step to machine
                        maOpt.ifPresent(aa -> {

                            //match allocation request's machine and stepId with order process capability invocation machine and stepId , if matches then request machine for order
                            if (aa.getValue().getId().equals(machineId) && aa.getKey().getID().equals(stepId)) {
                                ...
                                if (!currentLoc.isPresent()) {
                                    //	assign inputstation
                                    Optional<AkkaActorBackedCoreModelAbstractActor> inputStation = tryReserveNewProcessAtInputStation(orderId);
                                    if (!inputStation.isPresent()) {
                                        ...
                                        log.info(String.format("Order %s and potential (not yet requested) machine %s waiting for next available inputstation", orderId, aa.getValue().getId()));
                                        allocationRequest.setOrderAllocationResponse(String.format("Order %s and  machine %s waiting for next available inputstation", orderId, aa.getValue().getId()));
                                        //user has to reallocate when there is an ip station available,try later
                                    }
                                    // current loc is empty and input station is present
                                    if (inputStation.isPresent()) {

                                        try {
                                            ordMapper.requestMachineForOrder(aa.getValue(), orderId, aa.getKey());
                                            allocationRequest.setOrderAllocationResponse(String.format("%s is requested for executing order %s ", aa.getValue().getId(), orderId));
                                        ...
                                        ordMapper.scheduleProcess(orderId);
                                        // get machine actor reference
                                        log.info(String.format("Order %s about to be registered at machine %s", orderId, aa.getValue().getId()));
                                        aa.getValue().getAkkaActor().tell(new RegisterProcessStepRequest(orderId, aa.getKey().toString(), aa.getKey(), this.self()), this.self());
                                        allocationRequest.setOrderAllocationResponse(String.format("%s is registered at machine %s ", orderId, aa.getValue().getId()));
                                    }

                                }
                                //if currentLocation is present & input station is present
                                else {
                                    // order is at machine, we now register/request that machine
                                    try {
                                        ordMapper.requestMachineForOrder(aa.getValue(), orderId, aa.getKey());
                                        allocationRequest.setOrderAllocationResponse(String.format("%s is requested for executing order %s ", aa.getValue().getId(), orderId));
                                ...
                                allocationRequest.setOrderAllocationResponse(String.format("OrderProcess CapabilityInvocation step matches with the machine %s with step id %s , but provided machine and stepId are %s and %s , Order Allocation is not possible", aa.getValue().getId(), aa.getKey().getID(), machineId, stepId));

                            }
                        });

                        if (!maOpt.isPresent()) {
                            log.info(String.format("OrderProcess %s has no available CapabilityInvocations steps that match an available Machine/Actor, Order is paused", orderId));
                            ordMapper.pauseOrder(orderId);
                            allocationRequest.setOrderAllocationResponse(String.format("OrderProcess %s has no available CapabilityInvocations steps that match an available Machine/Actor, Order is paused ", orderId));
                        }
                    }
        ...
        allocationRequest.setStepId(stepId);
        allocationRequest.setMachineId(machineId);

        //add the order allocation responses to list and send to ActorRestEndPoint class
        orderAllocationResponse.add(allocationRequest);
        sender().tell(orderAllocationResponse, self);
...
//	}
    ...
    private Boolean tryAssignExecutingMachineForOneProcessStep(OrderProcess op, String rootOrderId) {
